...
func main() {
	...
	handler := func(msg message.Message[*pb.ProtoPayload]) error {
		messageCount++
		fmt.Printf("Mensagem recebida (Protobuf) #%d: %+v\n", messageCount, msg)
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		default:
			err := consumer.ConsumeMessage(ctx, topic, enums.ProtobufDeserialization, enums.OnDeserializationIgnoreMessage, handler)
			if err != nil {
